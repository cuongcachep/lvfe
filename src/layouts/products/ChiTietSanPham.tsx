...
const ChiTietSanPham: React.FC = () => {
  ...
  // Nút "Mua ngay":
  // - Tạo một mục giỏ tạm thời chỉ chứa sản phẩm hiện tại với số lượng đã chọn
  // - Lưu vào localStorage để các trang thanh toán đọc được
  // - Nếu đã đăng nhập (có JWT) → chuyển đến trang ThanhToan
  //   Nếu chưa đăng nhập → chuyển đến trang DatHangNhanh (điền nhanh thông tin)
  const handleMuaNgay = () => {
    if (!sach) return;
    ...
    // Lưu giỏ hàng tạm thời để trang ThanhToan/DatHangNhanh sử dụng
    localStorage.setItem('gioHang', JSON.stringify([sanPhamMuaNgay]));

    if (localStorage.getItem('jwt')) {
      // Đã đăng nhập → sang trang thanh toán đầy đủ
      navigate('/thanh-toan');
    } else {
      // Chưa đăng nhập → sang luồng đặt hàng nhanh (điền thông tin nhận hàng)
      navigate('/dat-hang-nhanh');
    }
  };


  //lấy mã sách từ tham số URL
