...
export async function findByBook(
  ...
  let duongDan: string = `http://localhost:8080/api/sach?sort=maSach,desc&size=8&page=${trangHienTai}`;
  
  if (tuKhoaTimKiem !== "" && maTheLoai == 0) {
    // Tìm theo tên sách
    duongDan = `http://localhost:8080/api/sach/search?tensach=${tuKhoaTimKiem}&page=${trangHienTai}&size=8`;
  } else if (tuKhoaTimKiem === "" && maTheLoai > 0) {
    // Tìm theo thể loại - ENDPOINT MỚI
    duongDan = `http://localhost:8080/api/sach/the-loai/${maTheLoai}?page=${trangHienTai}&size=8`;
  } else if (tuKhoaTimKiem !== "" && maTheLoai > 0) {
    // Tìm theo cả tên và thể loại
    duongDan = `http://localhost:8080/api/sach/the-loai/${maTheLoai}/search?tensach=${tuKhoaTimKiem}&page=${trangHienTai}&size=8`;
  }
...
export async function getAllTheLoai(): Promise<TheLoaiInterface[]> {
  const duongDan = `http://localhost:8080/api/the-loai`;
  const ketQua: TheLoaiInterface[] = [];
  ...
  try {
    ...
    // Nếu API trả về object có content property (pagination)
...
export async function getSachByTheLoai(
  maTheLoai: number,
  trangHienTai: number = 0
): Promise<KetQuaInterface> {
  const duongDan = `http://localhost:8080/api/sach/the-loai/${maTheLoai}?page=${trangHienTai}&size=8`;
  return laySach(duongDan);
}